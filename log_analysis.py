...
query2 = '''select rep.name, sum(views) as views from
            (select foo.path, foo.views, name, authors.id from
            (select log.path as path, count(*) as views from articles, log
            where concat('/article/',articles.slug) = log.path and
            log.status = '200 OK' group by log.path)foo, articles, authors
            where
            concat('/article/',articles.slug) = foo.path and
            authors.id = articles.author)rep
            group by rep.name order by views desc;'''
